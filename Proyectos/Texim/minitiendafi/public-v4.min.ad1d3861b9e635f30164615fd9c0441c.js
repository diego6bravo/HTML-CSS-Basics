...
if(isUnifiedLoginSite()){$.removeCookie("loginUserinfo",{path:"/"+window.digitalData.page.pageInfo.siteCode2+"/"});
...
$.removeCookie("agree-privacy-policy",{path:"/"});
if(sysConfig.countryCode=="FR"||sysConfig.countryCode=="DE"){$.removeCookie("loginUserinfo",{path:"/"+window.digitalData.page.pageInfo.siteCode2+"/"});
$.removeCookie("cbgLoginTag",{path:"/"+window.digitalData.page.pageInfo.siteCode2+"/"});
sessionStorage.getItem("ec-loginStatus")?sessionStorage.removeItem("ec-loginStatus"):"";
$.removeCookie("login_status",{path:"/"});
$.removeCookie("forumLogin",{path:"/"});
...
}}function getLoginUri(a,d){if(casUrl==null||casUrl==""){return""
...
}}var h=cmktUrl+"/login/cloudplatforms?siteURL="+encodeURIComponent(g);
...
}if(location.href.indexOf("community")!==-1&&location.href.indexOf("community/login")===-1&&!getCookie("loginUserinfo")&&!getCookie("forumLogin")){$.cookie("forumLogin","1",{path:"/"});
try{goLogin()
}catch(b){}}if(!sessionStorage.getItem("ec-loginStatus")){ecomLogin()
}}else{if((isECommerceSite==="Self-eCommerce"||isECommerceSite==="Fusion-eCommerce")&&typeof ecCom!="undefined"){var a=ecCom.get("guest_mode")&&ecCom.get("guest_mode")===1?true:false;
if((!a&&!!sessionStorage.getItem("ec-loginStatus"))||$.cookie("loginUserinfo")){if(casUrl!=null&&casUrl!=""){$.ajax({type:"get",dataType:"jsonp",url:getLoginUri(getNewUserUrl(),false),complete:function(){isCompleteUserInfo(null,null,false);
loginstate=-1;
getHuaweiAccountByRetry()
}})
}}if(!a&&!!sessionStorage.getItem("ec-loginStatus")){if(location.href.indexOf("community")!=-1&&!getCookie("loginUserinfo")&&!getCookie("forumLogin")){$.cookie("forumLogin","1",{path:"/"});
try{goLogin()
}catch(b){}}}if(!sessionStorage.getItem("ec-loginStatus")&&$.cookie("loginUserinfo")){ecomLogin()
...
if((!a&&!!sessionStorage.getItem("ec-loginStatus"))||$.cookie("loginUserinfo")||$.cookie("_ext_u_e_")){$.ajax({type:"get",dataType:"jsonp",url:getLoginUri(getNewUserUrl(),false),complete:function(){isCompleteUserInfo(null,null,false);
loginstate=-1;
getHuaweiAccountByRetry()
}})
...
}}function isCompleteUserInfo(d,b,c){if(!isShowLogin()){return
...
}}function trackCcpcCardDisplay(b,k,j,l){if(dmpa_site_id==dmpa_us_site_code){return
...
}catch(h){}if(typeof dmpa==="function"){dmpa("setAutoSendPV",false);
dmpa("set",{siteId:"consumer-cn",user_id:""});
dmpa("setAutoLinkTracking",false)
}dmpa("trackEvent","cardInfoDisplay",{data:"'"+d+"'"})
}function getAnalyticsType(b){var a="";
...
if(!a||!a.isTrusted||!(a.target==k)){return
...
function gaAndAaByAttr3(k,j,h,l,c,d,b,a,g,f){var h=h?getGAData(h):"Error: Not Maintain Value";
var j=j?getGAData(j):"Error: Not Maintain Value";
var k=k?getGAData(k):"Error: Not Maintain Value";
k=k+"+"+sysConfig.countryCode+"+"+sysConfig.language;
var l=l?l:null;
...
var g=g?g:null;
var f=f?f:null;
if(typeof ga==="function"){ga("newhuaiweisupport.send","event",k,j,h,{dimension7:l,dimension8:c,dimension9:d,dimension10:b,dimension11:a,dimension12:g,dimension13:f});
ga("send","event",k,j,h)
...
if(!a||!a.isTrusted||!(a.target==j)){return
...
haDispach(a);
gaAndAaByAttr(b,d,f,"","","")
}(function(b){var a={fr:"€",de:"€",es:"€",it:"€",uk:"£",nl:"€",jp:"¥",ph:"₱",pl:"zł",ru:"₽",th:"บาท",za:"R",mx:"$",co:"$",cl:"$",cz:"CZK",sa:"ر.س.","sa-en":"SAR",my:"RM",tr:"TL",eg:"جنيه","eg-en":"EGP",ae:"د.إ.","ae-en":"AED",pe:"S/",ro:"Lei"};
function c(){var d="";
try{if(b.siteCode){d=a[b.siteCode.toLowerCase()]
}}catch(f){return""
}return d?d:""
}b.getCurrencySymbol=c
})(window);
...
$(window).on("resize",function(){if($(".huawei-convergent-cookie").length>0){showJpCookie()
}else{showCookieTip()
}});
...
}else{$(".custom-service").css("bottom",d+22+"px");
...
var buyFeature=function(f,v,A,g){var k=f||$(this);
var x=k.attr("data-buybtntype");
var n=k.attr("data-pid");
var o=k.attr("data-pname");
var y=k.attr("data-targettype");
var j=k.attr("data-eventtype");
var B=k.attr("data-position");
var d=window.digitalData.page.pageInfo.siteCode2;
if(k.hasClass("eCommerce-buy")){var b=k.data("ecbuylink"),u=k.attr("data-ecpid"),r=k.data("ecopeninnewpage"),z=k.attr("data-btnlinkinfo"),h=k.attr("data-buttonType");
var a=r?"_blank":"_self";
if(h=="explore"&&z){fun_ecommerce(a,z,d)
}else{buyToThirdParty_gtm(g,B,j,k);
fun_ecommerce(a,b,d)
}return
}if(x=="none"){buyToPdp_gtm(g,B,j);
var b=k.attr("data-btnlinkinfo");
if(b.indexOf("/content/huawei-cbg-site")==0){b=b.replace("/content/huawei-cbg-site","")
}if(y=="_self"){window.location.href=b
}else{window.open(b)
}}else{if(x=="partner"){buyOpenPopup_gtm(g,B,j);
var w=k.attr("data-pcountrybeal")=="true"?true:false;
var q=k.attr("data-pcountrytitle");
q=!!q?q:Mkt.I18n.get("Select Country");
createPopupToBuy();
var p=v&&v.showStoreFinder;
var c="_self";
var t="";
if(p){c=v.openStoreFinderInNewPage?"_blank ":"_self";
t=v.storeFinderLink
}var C=k.attr("data-btnlinkinfo");
if(C.indexOf(".html")>0){C=C.substring(0,C.lastIndexOf(".html"))
}var l=C+"/_jcr_content.partners-v4.json";
fun_partner(l,o,y,p,t,c,w,q,A,g,j,B)
}else{if(x=="third-party-site"){var b=k.attr("data-btnlinkinfo");
if(b&&b.indexOf(window.location.origin+window.location.pathname)!=-1){j="product-detail-footer"
}buyToThirdParty_gtm(g,B,j,k);
fun_thirdParty(y,b,d)
}else{if(x=="global-buy"){buyOpenPopup_gtm(g,B,j);
if(!!n){createPopupToBuy();
fun_globalBuy(n,o,y,d,g,j,B);
...
var getMinPriceAndInv=function(b){var a=$.Deferred();
var c={};
c.api="queryMinPriceAndInv";
c.type="GET";
c.data={};
c.data.productIds=b;
c.success=function(d){a.resolve(d)
};
c.error=function(g,d,f){a.reject(g,d,f)
};
ecCom.ajaxReq(c);
return a.promise()
};
...
}function j(w,p){var y="";
var t="";
if(showFrom&&!p.onePriceDisplay){if(putFromFront){t=ecCom.I18n.get("ec_from")+" "
}else{y=" "+ecCom.I18n.get("ec_from")
}}var q="";
var o="";
if(w.installmentInfos){q="has-installment";
var r=w.installmentInfos.sort(function(A,z){return z.num-A.num
})[0].num;
...
}}var n=ecCurrency(w.minUnitPrice);
var v=ecCurrency(w.minOrderPrice);
var x=w.minUnitPrice==w.minOrderPrice?"":t+v+y;
var u="";
if(window.innerWidth>1199.98){u='<div class="ec-price"><span class="from-front">'+t+'</span><p class="sale-price">'+n+'<span class="from-behind">'+y+'</span></p><span class="installment '+q+'">'+o+'</span><span class="original-price">'+x+"</span></div>"
}else{u='<div class="ec-price"><p class="sale-price"><span class="from-front">'+t+"</span>"+n+'<span class="from-behind">'+y+'</span><span class="installment '+q+'">'+o+'</span></p><span class="original-price">'+x+"</span></div>"
}return u
}};
var ecEnableEcommerceLogic=function(){var a=$(".series-list-component,.model-list-component,.psp-list-component").find(".global-buy-button");
