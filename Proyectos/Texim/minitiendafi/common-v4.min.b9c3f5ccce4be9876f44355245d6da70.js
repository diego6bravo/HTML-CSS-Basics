...
}var buyToThirdParty_gtm=function(d,b,e,a){var c=GA_THIRD_PARTY[e];
...
$("[data-listtopdp]").on("click",function(){window.dataLayer.push(addATCommonFields({event:"listPdp",productMktName:$(this).attr("data-listtopdp"),productPosition:$(this).attr("data-position"),clickName:"product list_click to pdp_"+$(this).attr("data-position"),clickType:"action"}));
dmpaCommon("trackEvent","click","product list interaction",getATPageName(),$(this).attr("data-listtopdp"))
});
...
this.$platformToggle.on("click",function(){var c=b.$ipJump.innerHeight();
if(!$(this).hasClass(b.platformToggleDisabledClass)){if(!$(this).hasClass(b.platformToggleOpenedClass)){$(this).addClass(b.platformToggleOpenedClass);
b.$selector.addClass(b.platformSelectionOpenedClass);
if(b.$ipJump.is(":visible")){if($("#locationSecret").is(":visible")){var c=b.$ipJump.innerHeight();
var d=$("#locationSecret").height();
$(".platform-selection-mob").css("height","calc( 100% - "+(c+d)+"px)")
}else{$(".platform-selection-mob").css("height","calc( 100% - "+c+"px)")
}}else{if($("#locationSecret").is(":visible")){$(".platform-selection-mob").css("height","calc( 100% - 70px)")
}}b.closeFilter();
setTimeout(function(){b.$platformContent.stop().slideDown(500);
...
b.$platformContent.stop().slideUp(400,function(){b.$selector.removeClass(b.platformSelectionOpenedClass)
});
if(window.innerWidth<1200){if(b.$ipJump.length>0){$(".platform-selection-mob").removeAttr("style")
}}}}})
...
}else{var j={showStoreFinder:i,openStoreFinderInNewPage:c,storeFinderLink:b};
buyFeature($(this),j,f,$(this).attr("data-pname"));
...
if($(window).width()<992&&$(this.uspList)&&!$(this.uspList).hasClass("swiper-container-initialized")&&$(".usp-list__container .usp-list__item").length>1){new Swiper(a.uspList,{speed:600,loop:true,pagination:{el:".usp-list .swiper-pagination",clickable:true}})
...
}var PlpI18n={i18n_Buy:"",i18n_Explore:"",i18n_LearnMore:"",i18n_NoPrice:"",i18n_WhereToBuy:"",i18n_from:"",i18n_OperationFailed:""};
if(isECommerceSite&&isECommerceSite!="None"){PlpI18n.i18n_Buy=ecCom.I18n.get("ec_buy"),PlpI18n.i18n_Explore=ecCom.I18n.get("ec_explore"),PlpI18n.i18n_LearnMore=ecCom.I18n.get("ec_learn_more"),PlpI18n.i18n_NoPrice=ecCom.I18n.get("ec_no_price"),PlpI18n.i18n_WhereToBuy=ecCom.I18n.get("ec_where_to_buy"),PlpI18n.i18n_from=ecCom.I18n.get("ec_from"),PlpI18n.i18n_OperationFailed=ecCom.I18n.get("ec_operation_failed")
}function init_plpPrice(e,a,i,b){var c=e;
var f={},d="",h="";
if(b==="plp_banner"){d=c.parents(".product-listing-hero__container");
h=d.find(".product-listing-hero__buttons a.global-buy-button")
}else{if(b==="plp_productList"){d=c.parents(".product-col .product-block");
h=d.find(".product-block__details button")
}else{if(b==="compare"){d=c.parents(".hero-compare__product");
h=d.find(".hero-compare__links a.product_buy_btn")
}else{if(b==="pdp_relate"){d=c.parents(".relate-product-main .product-li");
h=d.find(".buy-common-btn button")
}else{if(b==="series_design"){d=c.parents(".series-main .series-product .series-product-list");
h=d.find(".btn-container a.series-btn1")
}}}}}this_canBuy=h.attr("data-enableec"),this_colorName=c.attr("data-colorname")?c.attr("data-colorname"):"",this_colorPriceId=h.attr("data-ecpid");
if(this_colorPriceId&&this_canBuy=="true"&&a.minPriceByColors&&(this_colorPriceId==a.productId)){if(a.minPriceByColors&&a.minPriceByColors.length>0){var g;
a.minPriceByColors.some(function(l,k){g=$.trim(l.color)==$.trim(this_colorName);
return g
});
$.each(a.minPriceByColors,function(m,k){var l=k.installmentInfos;
if(g){if($.trim(this_colorName)==$.trim(k.color)){if(l&&l.length>0){f.hasStaging=l.sort(function(n,j){return j.num-n.num
})[0]
}f.minUnitPrice=k.unitPrice;
f.minOrderPrice=k.orderPrice
}}else{if(l&&l.length>0){if(a.installmentInfos&&a.installmentInfos.length>0){f.hasStaging=a.installmentInfos.sort(function(n,j){return j.num-n.num
})[0]
}}f.minUnitPrice=a.minUnitPrice;
f.minOrderPrice=a.minOrderPrice
}})
}price_show(d,f,a,i,b,h)
}else{}}function price_show(i,j,k,c,h,q){var n="";
var p="";
var o=showFrom&&q.data("onepricedisplay")!==true;
var d=o?'<span class="text-from"> '+PlpI18n.i18n_from+" </span>":"";
if(o){if(putFromFront===true){n=d
}else{p=d
}}var r={};
...
}if(c){if(j){r.num=j.hasStaging.num?j.hasStaging.num:hasStaging.num
}else{r.num=hasStaging.num
}var g=ecCom.I18n.get("ec_payment_in_installments",[r.num]),b=window.innerWidth>=1024?"PC":"mobile",a=site==="UK"||site==="DE"||site==="ES",f=ecCom.I18n.get("ec_installment_or")+" "+g,m=ecCom.I18n.get("ec_installment_or")+" "+ecCom.I18n.get("ec_finance_available"),l=a?m:f;
if(b=="mobile"&&a){$(".plp-item button.a-common.plp-item-productbtn").css("margin-top","19px")
}else{if(b=="mobile"&&!a){$(".plp-item button.a-common.plp-item-productbtn").css("margin-top","2px")
}}priceHtml=r.minUnitPrice!=r.minOrderPrice?('<b class="plp-item-price"><ul class="proPrice"><li class="minorderprice"><span class="plp-item-minorderprice" style="margin-left: 0">'+n+" "+ecCurrency(r.minOrderPrice)+" "+p+'</span></li><li class="salePrice">'+n+"<span >"+ecCurrency(r.minUnitPrice)+" </span>"+p+'</li><li class="amountPrice"><span >'+l+"</span></li></ul></b>"):('<b class="plp-item-price"><ul class="proPrice"><li class="minorderprice"><span class="plp-item-minorderprice"></span></li><li class="salePrice">'+n+"<span >"+ecCurrency(r.minUnitPrice)+" </span>"+p+'</li><li class="amountPrice"><span >'+l+"</span></li></ul></b>")
}else{priceHtml=r.minUnitPrice!=r.minOrderPrice?('<b class="plp-item-price"><ul class="proPrice"><div class="inShow"><li class="salePrice">'+n+"<span >"+ecCurrency(r.minUnitPrice)+"</span>"+p+'</li><li class="minorderPrice" ><span class="plp-item-minorderprice">'+n+" "+ecCurrency(r.minOrderPrice)+" "+p+"</span></li></div></ul></b>"):('<b class="plp-item-price"><ul class="proPrice"><div class="inShow"><li class="minorderPrice" ><span ></span></li><li class="salePrice">'+n+"<span >"+ecCurrency(r.minUnitPrice)+"</span>"+p+"</li></div></ul></b>")
}if(i){i.find(".plp-item-fromprice").html(priceHtml)
...
if(d.length!=0){getMinPriceAndInv(d).then(function(w){if(w.resultCode===0){var v=w.data.minPriceAndInvList;
var s=new Map();
var j=[],t,h={},o={},f=new Map();
if(v){$.each(v,function(y){s.set(v[y].productId+"",v[y]);
j=v[y].installmentInfos;
if(j&&j.length>0){$.each(j,function(z,i){f.set(v[y].productId+"",v[y].installmentInfos)
})
}else{console.log(v[y].productId+" ID产品  不存在  分期信息或超出分期限额")
}})
}var q={};
for(var r=0;
r<a.length;
...
}if(b==="plp_productList"){e=q.find(".product-block__details button");
q.addClass("plp-li-scrollShow")
}else{if(b==="plp_banner"){e=q.find(".product-listing-hero__buttons a.global-buy-button")
}else{if(b==="compare"){e=q.find(".hero-compare__links a.product_buy_btn")
}else{if(b==="pdp_relate"){e=q.find(".buy-common-btn button")
}else{if(b==="series_design"){e=q.find(".btn-container a.series-btn1")
}}}}}e.addClass("eCommerce-buy");
u=e.attr("data-ecpid");
x=e.attr("data-enableec");
e.removeClass("plp-item-explorebtn");
e.removeClass("plp-item-buybtn");
var p=s.get(u),n=f.get(u);
var g="";
if(u&&n){h.amount=Math.min.apply(Math,n.map(function(i){return i.amount
}));
o=n.sort(function(y,i){return i.num-y.num
})[0]
}if(x=="true"&&u&&p){if(p.hasInv){e.addClass("plp-item-buybtn eCommerce-buy");
...
e.siblings("a.product-block__btn").text(PlpI18n.i18n_Explore).attr("title",PlpI18n.i18n_Explore);
e.siblings("a.btn--arrow").text(PlpI18n.i18n_Explore).attr("title",PlpI18n.i18n_Explore);
e.parents(".series-product-list-detail").find(".series-btn2").text(PlpI18n.i18n_Explore).attr("title",PlpI18n.i18n_Explore);
if(p.showPrice==false){g='<b class="plp-item-price">'+PlpI18n.i18n_NoPrice+"</b>";
if(b==="compare"){e.parents(".hero-compare__product").find(".hero-compare__description").addClass("noPrice")
}}else{if(p.minUnitPrice){var k=p,m=n,l;
if(b==="plp_productList"){$.each($(".product-row .swatches__item").find("button"),function(z,y){$(y).click(function(){init_plpPrice($(this),k,m,b)
})
});
$.each($(".product-wrap .swatches .swatches__wrapper"),function(y,i){init_plpPrice($(i).find("div").first().find("button"),k,m,b)
});
if($(".product-block__details").length>0&&device=="mobile"){$.each($(".product-block__details"),function(y,i){init_plpPrice($(i),k,m,b)
})
}}else{if(b==="plp_banner"){if($(".product-listing-hero__container .swatches__item").length>0){$.each($(".product-listing-hero__container .swatches__item"),function(z,y){$(y).click(function(){init_plpPrice($(this),k,m,b)
})
});
$.each($(".product-listing-hero__container .swatches .swatches__wrapper"),function(y,i){init_plpPrice($(i).find("div").first().find("button"),k,m,b)
})
}else{init_plpPrice(e,k,m,b)
}}else{if(b==="compare"){$.each($(".hero-compare__product-item .swatches__item"),function(z,y){$(y).click(function(){init_plpPrice($(this),k,m,b)
})
});
$.each($(".hero-compare__product-item .swatches__wrapper"),function(y,i){init_plpPrice($(i).find("div").first().find("button"),k,m,b)
})
}else{if(b==="pdp_relate"){$.each($(".swatches__swatch"),function(z,y){$(y).click(function(){init_plpPrice($(this),k,m,b)
})
});
$.each($(".relate-product-main .product-li .colorlist"),function(y,i){init_plpPrice($(i).find("li").first().find("button"),k,m,b)
})
}else{if(b==="series_design"){$.each($(".series-product-list-detail"),function(y,i){init_plpPrice($(i),k,m,b)
})
...
e.attr({buybtnCat:"pop_cta_click",act:"learn_more_button"})
}}else{e.addClass("plp-item-explorebtn");
e.attr("data-buttonType","explore");
e.html(PlpI18n.i18n_Explore).attr("title",PlpI18n.i18n_Explore);
e.parents(".product-listing-hero__container .product-listing-hero__buttons").find("a.banner-block__btn").remove();
...
if(errorCodeMap[w.resultCode]){w.message=ecCom.I18n.get(errorCodeMap[w.resultCode])
}console.error(w.message+" : queryMinPriceAndInv")
...
a=c.find(".product-listing-hero__container a.banner-block__btn")
...
}}}}}f.html(PlpI18n.i18n_Explore).attr({"data-buttonType":"explore",title:PlpI18n.i18n_Explore}).addClass("eCommerce-buy").removeClass("plp-item-buybtn").addClass("plp-item-explorebtn");
if(a){a.remove()
...
if(isECommerceSite=="Self-eCommerce"||isECommerceSite=="Fusion-eCommerce"){loadEcData(a,$(".relate-product-main .product-li").first(),"pdp_relate")
...
if(isECommerceSite=="Self-eCommerce"||isECommerceSite=="Fusion-eCommerce"){loadEcData(a,$(".series-main .series-product .series-product-list").first(),"series_design")
}}if($(".product-tabs-wrap .product-tabs__button-container").length>0&&isECommerceSite!="None"){$(".product-tabs-wrap .product-tabs__button-container").find("button.product-tabs__button").addClass("eCommerce-buy");
$(".product-tabs-wrap .product-tabs__button-container").on("click",function(){var a=$(this).find("button.product-tabs__button").attr("data-storelink")!=undefined?$(this).find("button.product-tabs__button").attr("data-storelink"):"";
buyFeature($(this).find("a.product_buy_btn"))
})
}setTimeShowBanner();
setBanenrBg();
toOtherPage();
function setTimeShowBanner(){if($(".banner-start-time").val()){var a=new Date().getTime();
$.ajax({type:"get",async:false,timeout:30000,url:"/services/server/time/?_="+new Date().getTime(),dataType:"json",success:function(j){a=j.timeStamp
}});
var b=($(".banner-start-time").val().substring(0,16)+":00").replace(/-/g,"/");
var d=($(".banner-end-time").val().substring(0,16)+":00").replace(/-/g,"/");
var i=$(".set-time_zone").val();
var g=-new Date().getTimezoneOffset()/60;
var c=(g-i)*60*60*1000;
var h=new Date(b).getTime()+c;
var e=new Date(d).getTime()+c;
if(h<=a&&a<e){$(".n5-header-banner").css("display","block");
f()
}else{$(".n5-header-banner").css("display","none")
}}function f(){$(".header-banner__close-btn").on("click",function(){event.stopPropagation();
$(".n5-header-banner").fadeOut(800);
var j={event:"adBannerUnderNavi",bannerName:"Close"};
window.dataLayer.push(addATCommonFields(j));
dmpaCommon("trackEvent","click","banner interaction","Close",$(".n5-header-banner").attr("href"))
})
}}function setBanenrBg(){var b=$(".header-begin-color").val();
var a=$(".header-end-color").val();
if(b&&!a){$(".n5-header-banner .main-header-banner").css("background",b)
}else{if(!b&&a){$(".n5-header-banner .main-header-banner").css("background",a)
}}}function toOtherPage(){var a=$(".header-banner-link").val();
if(a){$(".n5-header-banner").addClass("hand-shape");
var b=$(".header-banner-open-type").val();
b?b=b:b="_self";
$(".n5-header-banner").on("click",function(){event.stopPropagation();
window.open(a,b);
var d={event:"adBannerUnderNavi",bannerName:"Header Component V4_"+$($(".header-banner-text span")[1]).text()};
window.dataLayer.push(addATCommonFields(d));
var c="n5-header-banner_"+$(".header-banner-text span").eq(1).text();
dmpaCommon("trackEvent","click","banner interaction",c,$(this).attr("href"))
})
}}$(".n5-header-banner span a").on("click",function(){event.stopPropagation()
});